...
function syncObservationPhase(){
  document.querySelectorAll('[data-observation-phase]').forEach(button=>
    button.setAttribute('aria-pressed',String(button.dataset.observationPhase===state.observationPhase)));
}
function updateUI(p){
...
function bind(){
  ...
  $('play').onclick=()=>{if(!ready)return;if(state.cycles*strideFor(state.config)>=20)reset();state.running=!state.running;state.stepTarget=null;state.observationPhase=null;syncObservationPhase();playback();};
  $('step').onclick=()=>{if(!ready)return;if(state.cycles*strideFor(state.config)>=20)reset();state.stepTarget=state.cycles+0.5;state.running=true;state.observationPhase=null;syncObservationPhase();playback();};
  ...
  $('settings-file').onchange=async e=>{
    ...
    try{
      ...
      pause();state.config=data.config;state.cycles=data.cycles;state.a=null;state.b=null;state.viewingA=false;state.observationPhase=null;
