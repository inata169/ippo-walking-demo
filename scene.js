...
export function buildAvatar(scene){
 ...
 function update(p,c){
   ...
   pelvis.position.copy(h);pelvis.rotation.z=-lean*.4;
   ...
   for(const side of ['right','left']){
     const s=p[side],sign=side==='right'?1:-1,leg=legs[side],a=V(s.x,s.y,s.z),hip=h.clone().add(V(sign*.105,0,0)),k=V(...kneeBetween(hip.toArray(),a.toArray()));
     orient(leg.thigh,hip,k);orient(leg.shin,k,a);leg.knee.position.copy(k);leg.ankle.position.copy(a);leg.foot.position.copy(a);leg.foot.rotation.set(s.pitch-(c.ground==='slope'?Math.atan(.045):0),0,0);
...
export function createScene(canvas,config){
 ...
 function render(c,distance){if(c.ground!==ground){ground=c.ground;buildGround();}const p=samplePose(c,distance);avatar.update(p,c);const dz=distance-previousZ;camera.position.z+=dz;camera.position.y+=groundAt(distance,c.ground)-groundAt(previousZ,c.ground);controls.target.z+=dz;controls.target.y+=groundAt(distance,c.ground)-groundAt(previousZ,c.ground);previousZ=distance;sun.position.set(distance*.0-4,groundAt(distance,c.ground)+7,distance+4);sun.target.position.set(0,groundAt(distance,c.ground),distance);controls.update();renderer.render(scene,camera);return p;}
